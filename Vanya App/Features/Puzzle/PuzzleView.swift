            ...
            VStack(spacing: 0) {
                ...
                VStack(spacing: 12) {
                    ...
                    VStack(spacing: 6) {
                        ...
                        Text(unlocked ? "Masterpiece Restored" : "Tap pieces to swap and complete the artwork")
                            .font(.subheadline)
                            .foregroundColor(unlocked ? .green : .white.opacity(0.6))
                            .multilineTextAlignment(.center)
                    }
                    .padding(.top, 8)
                ...
                // 🧩 Award-Winning Puzzle Grid (Merging Logic Applied)
                LazyVGrid(columns: [
                    GridItem(.flexible(), spacing: unlocked ? 0 : 16),
                    GridItem(.flexible(), spacing: unlocked ? 0 : 16)
                ], spacing: unlocked ? 0 : 16) {
                    ForEach(pieces.indices, id: \.self) { index in
                        let piece = pieces[index]
                        let isCorrect = piece.correctIndex == index
                        let isSelected = selectedPiece?.id == piece.id
                        ...
                        ZStack {
                            ...
                            Image(piece.image)
                                ...
                                .scaledToFill()
                                .frame(height: 160)
                                // CORNER RADIUS MAPPING: Drops to 0 to form a single image
                                .clipShape(RoundedRectangle(cornerRadius: unlocked ? 0 : 20))
                                .opacity(showHint && !isCorrect ? 0.3 : 1)
                            
                            // Correct Glow - Only visible during gameplay
                            ...
                            // Hint number overlay
                            if showHint && !isCorrect && !unlocked {
                                ZStack {
                                    Circle()
                                        .fill(.ultraThinMaterial)
                                        .frame(width: 50, height: 50)
                        ...
                        .scaleEffect(isSelected ? 0.95 : 1.0)
                        // This makes the pieces slide and merge smoothly
                        .animation(.spring(response: 0.6, dampingFraction: 0.8), value: unlocked)
                        .onTapGesture {
                            if !unlocked {
                                handleTap(on: piece, at: index)
                            }
                ...
                .padding(.horizontal, unlocked ? 40 : 20)
                .animation(.spring(response: 0.6, dampingFraction: 0.8), value: unlocked)
                // Final presentation shadow
                .shadow(color: unlocked ? .green.opacity(0.3) : .clear, radius: 30)
                ...
                if unlocked {
                    NavigationLink {
                        ...
                        .background(
                            ZStack {
                                LinearGradient(
                                    colors: [Color(red: 0, green: 0.8, blue: 0.4), Color(red: 0, green: 0.65, blue: 0.35)],
                                    startPoint: .topLeading,
                                    endPoint: .bottomTrailing
                                )
                                LinearGradient(
    ...
    func checkCompletion() {
        ...
        if unlocked && !wasUnlocked {
            withAnimation(.spring(response: 0.6, dampingFraction: 0.7)) {
                showCelebration = true
                showHint = false // Auto-hide hints on completion
            }
            UIImpactFeedbackGenerator(style: .heavy).impactOccurred()
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                UIImpactFeedbackGenerator(style: .medium).impactOccurred()
